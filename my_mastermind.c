...
int *get_rnd_nums() {
    ...
    int gen_new_num[10] = {0}; // Modifié pour éviter les indices hors limites
...
int main(int argc, char **argv) {
    int *hiding_code = malloc(SAN_SIZE * sizeof(int));
    int my_gen_a_new_num = 10, rounds = 0;
    int *predit = malloc(SAN_SIZE * sizeof(int));

    if (argc > 2) {
        if ((my_get_digit(argv[2]) != 0) || (strlen(argv[2]) != SAN_SIZE)) {
    ...
    while (my_gen_a_new_num > 0) {
        ...
        char buffering[6]; // Augmenté la taille du tampon
